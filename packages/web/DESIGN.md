...
2. `/registry` (was `/`): the ledger table of skills. Columns: skill (name over mono id), author, escrowed on Arc, recorded off-chain, live claims, paid requests, disputes sustained/total, oldest claim, status stamp. Header shows total escrowed in `--seal`. No poll counter anywhere: react-query refetches in the background and the page never looks like it is reloading. Empty state: "Nothing listed yet." with a link to Register a skill.
