...
```mermaid
flowchart TB
  Web["Web app"] --> GW["Gateway<br/>(x402 service + verifier)"]
  Agent["Agent"] --> GW
  Agent --> SB["Sandbox<br/>(deny-by-default)"]
  Agent --> Circle["Circle Agent Stack wallet"]
  GW --> Hedera[("Hedera<br/>HCS · HTS · x402")]
  Circle --> Arc[("Arc<br/>SigilStake escrow")]
  GW --> Arc
  Web --> World["World<br/>Selfie Check"]
```
